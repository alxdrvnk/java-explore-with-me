...
class AdminCompilationControllerSpec extends Specification {
    ...
    def "Should return 201 when create compilation"() {
        ...
        def compilationMapper = Stub(CompilationMapper) {
            toNewCompilation(_ as NewCompilationDto) >> NewCompilation.builder().build()
            toCompilationDto(_ as Compilation) >> CompilationDto.builder().build()
        }
        ...
        then:
        interaction {
            1 * service.createCompilation(_ as NewCompilation) >> {
